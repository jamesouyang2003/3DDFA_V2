...
import cv2
import numpy as np
from math import cos, pi, sin, atan2, asin, sqrt
...
def viz_pose(img, param_lst, ver_lst, show_flag=False, wfp=None, print_angle=True, rotation=None):
	for param, ver in zip(param_lst, ver_lst):
		P, pose = calc_pose(param)
		# P = np.array([[ 9.33699548e-01, -2.25880057e-01,  2.77818799e-01,  5.59983597e+01],
		# 	[ 2.47910514e-01,  9.67030227e-01, -5.82483113e-02,  7.11895065e+01],
		# 	[-2.55502045e-01,  1.23260625e-01,  9.58913684e-01, -6.66714172e+01]])
		# P = np.array([[ 1, 0, 0,  5.59983597e+01],
		# 	[ 0,  1, 0,  7.11895065e+01],
		# 	[0,  0,  1, -6.66714172e+01]])
		if type(rotation) is np.ndarray:
			b = rotation[0] + 0.07 # yaw
			c = -rotation[1] - 0.32  # pitch
			a = rotation[2] + 0.07  # roll
			# from here: http://planning.cs.uiuc.edu/node102.html
			P = np.array([  [ cos(a)*cos(b), cos(a)*sin(b)*sin(c)-sin(a)*cos(c), cos(a)*sin(b)*cos(c)+sin(a)*sin(c),  5.59983597e+01],
							[ sin(a)*cos(b), sin(a)*sin(b)*sin(c)+cos(a)*cos(c), sin(a)*sin(b)*cos(c)-cos(a)*sin(c),  7.11895065e+01],
							[-sin(b), cos(b)*sin(c), cos(b)*cos(c), -6.66714172e+01]])
			# P = np.hstack((rotation, P[:,[3]])) # If using rotation matrix instead of axes
